...
// ===== Injetar GSAP (necessário para Winwheel) =====
...
winwheelScript.onload = () => {
  // Winwheel carregado, agora podemos inicializar quando necessário
};
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step3" class="step">
      ...
      <p style="margin-bottom:40px;">Toque em "Girar" e descubra seu amigo secreto!</p>
      <div class="spinner-container">
        <canvas id="spinner" width="320" height="320"></canvas>
...
function startSpin() {
  if (theWheel) {
    ...
    // Sorteio aleatório real: para em segmento aleatório
    const randomStop = Math.floor(Math.random() * spinnerParticipants.length) + 1;
    theWheel.stopAnimation(false);
    theWheel.rotationAngle = 0;
    theWheel.draw();
    theWheel.animation.spins = 10 + Math.floor(Math.random() * 5);
    theWheel.animation.stopAngle = randomStop * (360 / spinnerParticipants.length) + Math.floor(Math.random() * (360 / spinnerParticipants.length));
    theWheel.startAnimation();
  }
}

function revealResult(indicatedSegment) {
  drawnPerson = spinnerParticipants[indicatedSegment.segmentNumber - 1];
  launchConfetti();
  setTimeout(() => {
