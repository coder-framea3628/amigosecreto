...
// Lista para o spinner (fácil de editar/remover alguém para controle admin)
let spinnerParticipants = profiles.map(p => ({ ...p })); // cópia completa
...
function launchConfetti() {
  confetti({
    ...
    spread: 70,
    origin: { y: 0.6 },
    colors: ['#8b6f47', '#5e3f3b', '#3e5f47', '#c44c4c']
  });
}

// Spinner com Winwheel.js embutido (versão mínima)
function initSpinner() {
  const segments = spinnerParticipants.map(p => ({
    'text': p.name,
    'fillStyle': '#fdfaf7',
  ...
  window.theWheel = theWheel; // para acesso global no botão
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.profile-card').forEach(card => {
    card.addEventListener('click', () => {
      selectedProfile = {
        ...
        relation: card.dataset.relation,
  ...
  document.getElementById('confirm-yes').addEventListener('click', () => {
    ...
    showLoading();
    // Remove o próprio perfil do spinner
