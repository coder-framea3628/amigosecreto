...
.btn {
  ...
  font-size: 1.2rem;
  border: none;
  background: var(--accent); /* vinho para o botão Girar */
  color: #fff;
...
document.body.innerHTML = `
  ...
  <div class="bottom-sheet" id="confirm-popup">
    ...
    <button class="btn secondary" style="background:transparent;border:2px solid var(--accent);color:var(--accent);" id="confirm-no">Voltar</button>
  </div>
  ...
  <div class="bottom-sheet" id="result-popup">
    ...
    <button class="btn" id="show-code">Exibir código de segurança</button>
  </div>

  <div class="bottom-sheet" id="code-popup">
    <span class="close-popup">×</span>
    <h2>Confirmação final</h2>
    <p>Para garantir que tudo fique certinho e ninguém repita, envie quem você tirou pra mim (Luiza) pelo WhatsApp.</p>
    <p style="font-size:0.9rem;margin:20px 0;">É só tirar print dessa tela e mandar!</p>
...
document.addEventListener('DOMContentLoaded', () => {
  if (localStorage.getItem('amigoSecretoDone')) {
    document.querySelector('.container').innerHTML = `
      <h1>Ops! 😅</h1>
      <p>Você já realizou o seu sorteio do amigo secreto esse ano.</p>
      <p style="font-size:1rem;margin-top:32px;">Se precisar de algo, é só chamar a Luiza!</p>
      <div class="footer">
        Desenvolvido com <svg viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg> por Luiza Prado
      </div>
    `;
  ...
  document.getElementById('spin-btn').addEventListener('click', () => {
    ...
    // Simula sorteio com delay de 4 segundos
    setTimeout(() => {
      hideSpinning();
  ...
  document.getElementById('send-whatsapp').addEventListener('click', () => {
    const message = encodeURIComponent(`Olá Luiza! Tudo bem? Tirei ${drawnPerson.name} no amigo secreto da família Pereira esse ano. Aqui vai o print! 🎄`);
